  ...
  return `
    ...
    <nav class="sidebar-nav">
      ${SHELL_LINKS.map(link => `
        <a href="/${link.href.replace('.html','')}" class="nav-item ${link.href === activeHref ? 'active' : ''}" data-route="${link.href}">${link.label}</a>
      `).join('')}
    </nav>
...
async function renderProtectedPage({ title, activeHref, contentHtml, onMount, topbarHtml }) {
  ...
  if (!shell) { console.warn('Shared shell container not found'); return; }
